...
class DashboardController extends Controller
{
    public function index(){
        ...
        $beatCustomerDues = Beat::with(['customers.payments'])
            ->get()
            ->map(function ($beat) {
                return [
                    'beat_name' => $beat->beat_name,
                    'total_due' => $beat->customers->sum(function ($customer) {
                        return $customer->payments->sum('total_due') ?? 0;
                    })
                ];
            });    
        
        return view('pages.dashboard.index',compact('currentPage','beatsSum','total_pay','beatCustomerDues'));
    }
